...
export default function DoctorPage() {
  ...
  const primaryDoctorName = doctors && doctors.length > 0 ? doctors[0].name.split(' ')[0] : 'Doctor';
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      // 1. Today's Patients
      const { count: tCount } = await supabase.from('prescriptions').select('*', { count: 'exact', head: true }).eq('date', todayStr);
      
      // 2. Weekly Trend
      const { data: thisW } = await supabase.from('prescriptions').select('id').gte('date', last7DaysStr);
      const { data: prevW } = await supabase.from('prescriptions').select('id').gte('date', prev7DaysStr).lt('date', last7DaysStr);
      const trendVal = prevW?.length ? Math.round(((thisW!.length - prevW.length) / prevW.length) * 100) : 0;

      // 3. Today's Revenue
      const { data: receipts } = await supabase.from('receipts').select('total_amount, items_json').gte('printed_at', todayStr);
      ...
      const { data: recent } = await supabase.from('prescriptions').select('*, patients(name, gender, age)').order('created_at', { ascending: false }).limit(5);
  ...
  const quickActions = [
    { label: 'Register New Patient', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="23" y1="11" x2="17" y2="11"></line></svg>, href: '/portal/prescription' },
    { label: 'Digital Prescription', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>, href: '/portal/prescription' },
    ...
    { label: 'Clinical Patient History', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>, href: '/portal/doctor/patient-history' },
  ];

  return (
    <DashboardLayout>
      <div className={styles.dashboardHeader}>
        <h2>Welcome back, Dr. {primaryDoctorName}.</h2>
      ...
      <div className={styles.bentoGrid}>
          ...
          <div className={styles.sectionBox}>
            <div className={styles.sectionHeader}>
              <div>
                ...
                <p style={{ fontSize: 13, color: 'var(--sanctuary-ink-l)', marginTop: 4 }}>Recent Clinical Observances</p>
              </div>
            </div>
            ...
            <div className={styles.queueList}>
              {recentPatients.map((p) => (
                <div key={p.id} className={styles.queueItem}>
                  ...
                  <Link href={`/portal/doctor/patient-history?patientId=${p.patient_id}`} style={{ color: 'var(--sanctuary-ink-l)', textDecoration: 'none' }}>
              ...
              {recentPatients.length === 0 && (
                <div style={{ padding: '40px 0', textAlign: 'center', color: 'var(--sanctuary-ink-l)' }}>
                  <p>No patients observed yet in this cycle.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className={styles.sideCol}>
          <div className={styles.quickActions}>
            <h4 style={{ fontSize: 18, fontWeight: 800, color: 'var(--sanctuary-primary)', marginBottom: 16 }}>Clinical Portfolio</h4>
              ...
              {searchTerm.length >= 2 && (
                <div className={styles.searchResults}>
                  {isSearching ? (
                    ...
                    <>
                      {searchResults.map(p => (
                        <div key={p.id} className={styles.searchItem} onClick={() => window.location.href = `/portal/prescription?patientId=${p.id}`}>
                          <div>
